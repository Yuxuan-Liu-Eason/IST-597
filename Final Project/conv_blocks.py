...
Original file is located at
    https://colab.research.google.com/drive/1-drb5t-Y5J1t6VJ-u6ShiLzYgVoJLEEk
"""
...
class ResNetAttention(tf.keras.layers.Layer):
  """ResNet Attention layer"""

  def __init__(self, *args, **kwargs):
    super(ResNetAttention, self).__init__(*args, **kwargs)

  def build(self, input_shape):
    base_model = tf.keras.applications.ResNet152V2(include_top=False, weights='imagenet', pooling='avg') # use ResNet152 network with weights pretrained on ImageNet
    self.reduce_dim = tf.keras.layers.Conv2D(128, kernel_size=(1,1), padding='same')
    self.features_extraction = tf.keras.models.Model(inputs=base_model.input, outputs=base_model.get_layer('conv3_block1_2_conv').output) # extract ResNet features after the final avg pool block
    super(ResNetAttention, self).build(input_shape)
