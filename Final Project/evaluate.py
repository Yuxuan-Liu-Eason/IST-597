...
Original file is located at
    https://colab.research.google.com/drive/1nhv0znA95gTu_bJwi1OzaxIWIsQXM0lc
"""
...
import ljpeg, ljpeg_attention, ljpeg_PIL, ljpeg_res, ljpeg_res_attention
...
class Evaluator():
    ...
    def evaluate(self):
        ...
        if os.path.isfile(path):
            ...
            elif self.jpeg_type == 'ljpeg_res':
                model = ljpeg_res.JPEGAutoEncoder(training=False)
                x_tilde, qtables = model(x)
            elif self.jpeg_type == 'ljpeg_res_attention':
              model = ljpeg_res_attention.JPEGAutoEncoder(training=False)
              x_tilde, _, qtables = model(x)
            ...
            print("Evaluating ", self.jpeg_type)
            ...
            with tf.Session() as sess:
                ...
                if (self.jpeg_type == 'ljpeg' or self.jpeg_type == 'ljpeg_res' or self.jpeg_type == 'google' 
                    or self.jpeg_type == 'ljpeg_attention' or self.jpeg_type == 'ljpeg_attention_qtable' or self.jpeg_type == 'ljpeg_res_attention'):
...
def main(args):
    ...
    #JPEG Standard
    cmd = "mkdir -p {}".format(args.batch_dir + "/_output_PIL")
    os.system(cmd)
    bpp_avg_list = []
    mse_avg_list = []
    lpips_avg_list = []
    psnr_avg_list = []
    mssim_avg_list = []
    quality_list = list(range(1,10)) + list(range(10,20,5))
    for q in quality_list:
        ...
        bpp_avg = round(np.mean(bpp_array), 3)
        mse_avg = round(np.mean(mse_array), 3)
        lpips_avg = round(np.mean(lpips_array), 3)
        psnr_avg = round(np.mean(psnr_array), 3)
        mssim_avg = round(np.mean(mssim_array), 3)
    ...
    plt.plot(bpp_avg_list, mssim_avg_list, label='JPEG')
    print('SSIM: ', mssim_avg_list, '\nBPP: ', bpp_avg_list, '\nMSE: ', mse_avg_list, '\mLPIPS: ', lpips_avg_list)
    ...
    if args.google_dir is not None: # check if a path to a google baseline eexperiment was given in the arguments
        ...
        plt.plot(bpp_avg_list, mssim_avg_list, "-+", label='google')
        print('SSIM: ', mssim_avg_list, '\nBPP: ', bpp_avg_list, '\nMSE: ', mse_avg_list)

    #Experiment
    experiments = glob.glob(args.batch_dir + "/[!_]*")
    cmd = "mkdir {}".format(args.batch_dir + "/_output")
    os.system(cmd)
    print('Experiment: ', experiments)
    result_df = pd.DataFrame(columns=['experiment', 'lambda', 'bpp', 'mse', 'lpips', 'psnr', 'ms-ssim'])
    i = 0
    for experiment in experiments:
        runs = glob.glob(experiment + "/[!_]*")
        runs.sort()
        print('Run: ', runs)
        bpp_avg_list = []
        mse_avg_list = []
        ...
        result_df.loc[i] = [experiment[experiment.rfind('/')+1:], name_list, bpp_avg_list, mse_avg_list, lpips_avg_list, psnr_avg_list, mssim_avg_list]
        i += 1
        ...
        if runs:
            ...
            plt.plot(bpp_avg_list, mssim_avg_list, "-+", label=ex)
            print('SSIM: ', mssim_avg_list, '\nBPP: ', bpp_avg_list, '\nMSE: ', mse_avg_list, '\mLPIPS: ', lpips_avg_list)
            plt.xlabel("BPP")
            plt.ylabel("MSSIM [dB]")
            plt.legend()
    ...
    result_df.to_csv(experiment[:experiment.rfind('/')+1]+'result.csv', index=False)
