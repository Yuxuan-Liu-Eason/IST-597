# -*- coding: utf-8 -*-
"""ljpeg_attention.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1_RWJo-IO0rwH6bvP1er36ptb0_bJZbKJ
"""

"""Learned JPEG Module with attention.
...
class JPEGAutoEncoder(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.vgg_attention = VGGAttention() #ResNet attention network
    ...
    def call(self, tensor):
        ...
        print("image size: ", input_image.shape)
        tensor, attention = self.vgg_attention(tensor, input_image) #apply attention based smoothing
        ...
        return tensor, attention, self.quantizer_weights
...
def train(args):
    ...
    # Build autoencoder.
    x_tilde, attention_map, qtables = model(x)

    # Total number of bits divided by number of pixels.
    attention_map_mean = tf.reduce_mean(attention_map)
    ...
    #train_msssim = tf.reduce_mean(tf.image.ssim_multiscale(x, x_tilde, 1, filter_size=4))
    # Perceptual loss
    ...
    train_loss = args.lmbda * (
                train_mse + args.lpips_weight * lpips) + attention_map_mean + args.total_weight_regularizer * summed_weights
    ...
    # average attention maps over all frequencies for visualitzation
    attention_map_lum = tf.image.resize(tf.expand_dims(tf.reduce_mean(attention_map[:, :, :, :, :, 0], [-2, -1]), -1),
                                        [args.patchsize, args.patchsize])
    attention_map_chrom = tf.image.resize(tf.expand_dims(tf.reduce_mean(attention_map[:, :, :, :, :, 1], [-2, -1]), -1),
                                          [args.patchsize, args.patchsize])

    # Tensorboard
    tf.summary.scalar("loss", train_loss)
    tf.summary.scalar("attention map mean", attention_map_mean)
    tf.summary.scalar("true_bpp", bpp)
    tf.summary.scalar("mse", train_mse)
    #tf.summary.scalar("ssim", train_msssim)
    ...
    tf.summary.image("attention map luma", quantize_image(attention_map_lum))
    tf.summary.image("attention map chrominance", quantize_image(attention_map_chrom))
